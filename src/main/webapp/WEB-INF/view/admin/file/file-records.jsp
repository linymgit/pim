...
<script type="text/javascript">
        ...
        $.ajax({
            ...
            success: function (data, textStatus, jqXHR) {
                ...
                for (let i in l) {
                    ...
                    h += '<td style="width: 20%">' + l[i].userid + '</td>';
                    h += '<td style="width: 20%">' + l[i].resourceName + '</td>';
                    h += '<td style="width: 20%"><a onclick="openFile(\'' + l[i].resourceUrl + '\')">' + (host + l[i].resourceUrl) + '</a></td>';
                    h += '<td style="width: 20%">' + l[i].createTime + '</td>';
                }
                $("#yimi-table").append(h);
    ...
    function delFile() {
        ...
        if(sel===true) {
            $.ajax({
                type: 'post',
                url: '/admin2020/file/remove?id=' + gId,
                headers: {
                    'x-token': localStorage.getItem("x-token"),
                },
                success: function (data, textStatus, jqXHR) {
